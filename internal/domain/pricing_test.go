...
func TestCalculateVolumePrice(t *testing.T) {
	...
	if r1.DiscountPercent != 0 || r1.DiscountedUnitPrice != 100000 || r1.Subtotal != 100000 {
	...
	if r2.DiscountPercent != 5 || r2.DiscountedUnitPrice != 95000 || r2.Subtotal != 190000 {
	...
	if r5.DiscountPercent != 10 || r5.DiscountedUnitPrice != 90000 || r5.Subtotal != 450000 {
		t.Fatalf("expected 10%% discount for 5 units, got %v", r5)
	}

	// 12 units -> 15% discount
	r12 := domain.CalculateVolumePrice(basePrice, 12)
	if r12.DiscountPercent != 15 || r12.DiscountedUnitPrice != 85000 || r12.Subtotal != 1020000 {
		t.Fatalf("expected 15%% discount for 12 units, got %v", r12)
	}
}

func TestCalculateVolumePriceRoundsToTheNearestCent(t *testing.T) {
	result := domain.CalculateVolumePrice(1999, 2)
	if result.DiscountedUnitPrice != 1899 || result.Subtotal != 3798 || result.TotalSavings != 200 {
		t.Fatalf("unexpected rounded pricing: %#v", result)
	}
}
